...
// Only run subscriber once per testing session
// Use TEST_LOG=TRUE to display logs when testing
...
#[actix_rt::test]
async fn subscribe_returns_a_200_when_field_are_present_but_empty(){
    let app = spawn_app().await;
    let client = reqwest::Client::new();
    let test_cases = vec![
